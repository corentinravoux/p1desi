...
pk_means_name = os.path.join(
    path_to_pk, f"mean_Pk1d_par{'_vel' if velunits else ''}.fits.gz"
)
if substract_sb:
    pk_means_name_sb = os.path.join(
        path_sb_substract, f"mean_Pk1d_par{'_vel' if velunits else ''}.fits.gz"
    )


outname = os.path.join(
    f"{path_plot}_model{model_compare_str}_zmax{zmax}_unit{'kms' if velunits else 'A'}"
)

comparison_model_file = [
    "/global/homes/r/ravouxco/2_Software/Python/Data/p1d_models/models_eBOSS_lowz.fits",
...
kwargs_noise1 = {"ncol_legend": 2, "kmin": None, "kmax": None}
...
kwargs_noise2 = {"kmin": kmin, "kmax": kmax}
...
if __name__ == "__main__":
    ...
    if plot_noise_comparison_mean_z:
        noise.compute_and_plot_mean_z_noise_power(data, zbins, outname, **kwargs_noise2)
