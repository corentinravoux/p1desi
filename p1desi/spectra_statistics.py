...
def get_spectra_desi(
    ...
    for path in spectra_path:
        if spectro == "all":
            ...
            if compute_diff:
                exp_names = np.sort(
                    glob.glob(os.path.join(path, "*/spectra-{}-*.fits".format(spectro)))
                )
            spectra_names = np.sort(
                glob.glob(os.path.join(path, "*/coadd-{}-*.fits".format(spectro)))
            )
        ...
        for i in range(len(spectra_names)):
            ...
            for k in range(len(mask_desi_target_names)):
                if mask_desi_target_names[k] in spectra.fibermap.colnames:
                    for j in range(len(cut_objects)):
                        mask_target |= (
                            spectra.fibermap[mask_desi_target_names[k]]
                            & mask_desi_target[k][cut_objects[j]]
                        ) != 0
            ...
            if get_fuji_pb_spectra:
                mask_target = mask_target & (
                    (spectra.fibermap[name_fiberstatus] == 0)
                    | (spectra.fibermap[name_fiberstatus] == 8388608)
                    | (spectra.fibermap[name_fiberstatus] == 16777216)
                )
            else:
            ...
            if compute_diff:
                ...
                for band in ["b", "r", "z"]:
                    for j in range(len(target_id_to_append)):
                        mask_targetid = (
                            exp.fibermap["TARGETID"] == target_id_to_append[j]
                        )
...
def hist_profile_2d_bins(x, y, bins, statistic="mean", outlier_insensitive=False):
    ...
    bin_edges = np.array(
        [i * ((x[-1] - x[0]) / bins) + x[0] for i in range(bins)] + [x[-1]]
    )
    ...
    for i in range(len(bin_edges) - 1):
        ...
        if outlier_insensitive:
            if statistic == "mean":
                ...
                outlier_insensitive_std = (
                    lambda x: (
                        np.nanpercentile(x, 84.135) - np.nanpercentile(x, 15.865)
                    )
                    / 2
                )
...
def plot_variance_histogram(
    ...
    **kwargs,
    ...
    fontsize_legend = utils.return_key(kwargs, "fontsize_legend", 13)
    labelsize = utils.return_key(kwargs, "labelsize", 12)
    alpha = utils.return_key(kwargs, "alpha", 0.3)
    figsize = utils.return_key(kwargs, "figsize", (8, 7))
    ...
    bin_centers, means1, disp1 = hist_profile(
        wavelength,
        V_diff,
        nb_bins,
    ...
    bin_centers, means2, disp2 = hist_profile(
        wavelength,
        V_pipeline,
        nb_bins,
        (np.min(wavelength), np.max(wavelength)),
    ...
    ax[0].set_ylabel(r"$\overline{\mathbb{V}}_{i}$", fontsize=fontsize)
    ax[0].legend(["Diff", "Pipeline"], fontsize=fontsize_legend)
    ax[0].tick_params(axis="x", labelsize=labelsize)
    ax[0].tick_params(axis="y", labelsize=labelsize)

    (ratio_means, ratio_disp) = (
        (means1 - means2) / means2,
        (means1 / means2) * np.sqrt((disp1 / means1) ** 2 + (disp2 / means2) ** 2),
    )

    (diff_mean, diff_disp) = ((means1 - means2), np.sqrt((disp1) ** 2 + (disp2) ** 2))

    ratio = (V_diff - V_pipeline) / V_pipeline
    diff = V_diff - V_pipeline

    ax[1].plot(wavelength, diff, color="C3", alpha=alpha)
    ...
    bin_centers, means_diff, disp_diff = hist_profile(
        wavelength,
        diff,
        nb_bins,
        (np.min(wavelength), np.max(wavelength)),
    ...
    ax[1].legend(
        ["Difference", "Difference of bins", "Binned difference"],
        fontsize=fontsize_legend,
    )
    ax[1].set_ylabel(
        "$\overline{\mathbb{V}}_{\mathrm{diff}} - \overline{\mathbb{V}}_{\mathrm{pip}}$",
        fontsize=fontsize,
    )
    ...
    if plot_ratio:
        ax[2].set_xlabel("$\lambda_{\mathrm{obs}} [\AA]$", fontsize=fontsize)
    else:
        ax[1].set_xlabel("$\lambda_{\mathrm{obs}} [\AA]$", fontsize=fontsize)
    if plot_ratio:
        ax[2].plot(wavelength, ratio, color="C3", alpha=alpha)
        ...
        bin_centers, means_ratio, disp_ratio = hist_profile(
            wavelength,
            ratio,
            nb_bins,
            (np.min(wavelength), np.max(wavelength)),
        ...
        ax[2].legend(
            ["Ratio", "Ratio of bins", "Binned ratio"], fontsize=fontsize_legend
        )
        ax[2].set_ylabel(
            "$\overline{\mathbb{V}}_{\mathrm{diff}}  - \overline{\mathbb{V}}_{\mathrm{pip}} ) / \overline{\mathbb{V}}_{\mathrm{pip}}$",
            fontsize=fontsize,
        )
        ax[2].tick_params(axis="x", labelsize=labelsize)
        ax[2].tick_params(axis="y", labelsize=labelsize)
    ...
    if overlap_regions is not None:
        for i in range(len(overlap_regions)):
            overlap_bin_diff = np.mean(
                V_diff[
                    (wavelength > overlap_regions[i][0])
                    & (wavelength < overlap_regions[i][1])
                ]
            )
            overlap_bin_pipeline = np.mean(
                V_pipeline[
                    (wavelength > overlap_regions[i][0])
                    & (wavelength < overlap_regions[i][1])
                ]
            ...
            ax[0].plot(
                [np.mean(overlap_regions[i])], [overlap_bin_diff], marker=".", color="b"
            )
            ax[0].plot(
                [np.mean(overlap_regions[i])],
                [overlap_bin_pipeline],
                marker=".",
                color="r",
            )
            ax[0].fill_between(
                ...
                np.max(means1),
                where=(
                    (wavelength > overlap_regions[0][0])
...
def plot_variance(
    ...
    **kwargs,
    ...
    ax[0].plot(
        wavelength, V_diff, label="diff", color="b", linestyle=linestyle, marker=marker
    )
    ax[0].plot(
        wavelength,
        V_pipeline,
    ...
    ax[1].plot(
        wavelength, ratio, label="ratio", color="b", linestyle=linestyle, marker=marker
    )
    ...
    ax[2].plot(
        wavelength, diff, label="diff", color="b", linestyle=linestyle, marker=marker
    )
    ...
    if overlap_regions is not None:
        ax[0].fill_between(
            wavelength,
            min(np.min(V_diff), np.min(V_pipeline)),
            max(np.max(V_diff), np.max(V_pipeline)),
            where=(
                (wavelength > overlap_regions[0][0])
                & (wavelength < overlap_regions[0][1])
            )
...
def plot_hist(name_out, var_pipeline, diff, wavelength, nb_bins, **kwargs):

    V_diff = np.nanvar(diff, axis=0)
    V_pipeline = np.nanmean(var_pipeline, axis=0)

    plot_variance(f"{name_out}_raw", V_diff, V_pipeline, wavelength, **kwargs)

    plot_variance_histogram(
        f"{name_out}_histogram",
        V_diff,
        V_pipeline,
        wavelength,
        nb_bins=nb_bins,
    ...
    plot_variance_histogram(
        f"{name_out}_histogram_bin_outlier_insensitive",
        V_diff,
        V_pipeline,
        wavelength,
        nb_bins=nb_bins,
...
def plot_hist_var_outlier_insensitive(
    name_out, var_pipeline, diff, wavelength, nb_bins, **kwargs
):

    V_diff_outliers = (
        (
            np.nanpercentile(diff, 84.135, axis=0)
            - np.nanpercentile(diff, 15.865, axis=0)
        )
        / 2
    ) ** 2
    ...
    plot_variance(
        f"{name_out}_raw_var_outlier_insensitive",
        V_diff_outliers,
        V_pipeline_outliers,
        wavelength,
        **kwargs,
    ...
    plot_variance_histogram(
        f"{name_out}_histogram_var_outlier_insensitive",
        V_diff_outliers,
        V_pipeline_outliers,
        wavelength,
        nb_bins=nb_bins,
    ...
    plot_variance_histogram(
        f"{name_out}_histogram_var_outlier_insensitive_bin_outlier_insensitive",
        V_diff_outliers,
        V_pipeline_outliers,
        wavelength,
        nb_bins=nb_bins,
...
def plot_2d_hist(name_out, var_pipeline, diff, wavelength, nb_bins, **kwargs):

    bin_wave, V_pipeline = hist_profile_2d_bins(
        wavelength, var_pipeline, nb_bins, statistic="mean", outlier_insensitive=False
    )

    bin_wave, V_diff = hist_profile_2d_bins(
        wavelength, diff, nb_bins, statistic="var", outlier_insensitive=False
    )

    plot_variance(
        f"{name_out}_2d_histogram", V_diff, V_pipeline, bin_wave, points=True, **kwargs
    )

    bin_wave, V_pipeline = hist_profile_2d_bins(
        wavelength, var_pipeline, nb_bins, statistic="mean", outlier_insensitive=True
    )

    bin_wave, V_diff = hist_profile_2d_bins(
        wavelength, diff, nb_bins, statistic="var", outlier_insensitive=True
    )

    plot_variance(
        f"{name_out}_2d_histogram_outlier_insensitive",
        V_diff,
        V_pipeline,
        bin_wave,
        points=True,
...
def plot_noise_fluxvariance_comparison(
    V_coadd, V_pipeline, wavelength, name_out, nb_bins=40
):
    ...
    bin_centers, means, disp = hist_profile(
        wavelength,
        V_coadd,
        nb_bins,
        (np.min(wavelength), np.max(wavelength)),
...
def plot_noise_fluxvariance_ratio(
    name_out, spectra_array, noise_array, wavelength, nb_bins, outlier_insensitive=False
):
    mean_noise = []
    var_flux = []
