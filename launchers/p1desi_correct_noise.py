...
path_in =  "/global/cfs/cdirs/desi/users/ravouxco/pk1d/desi_data/fugu/p1d_healpix/v6/p1d_out/"
...
noise_corrections = [{#"sv1": (0.026,1.77,0.00076),
                      #"sv2": (0.0,0.0,0.00127),
                      "sv3": (0.0,0.0,0.00127),
...
if __name__ == "__main__":
    for i in range(len(regions)):
        ...
        for snr_cut in snr_cuts:
            ...
            folder_name = bookkeeping.return_folder_name(region,lines_name,catalog_name,dla_name,bal,noise_estimate,suffix)
            pk_in = bookkeeping.return_pk_path(path_in,folder_name,snr_cut)
            folder_name_out = bookkeeping.return_folder_name(region,lines_name,catalog_name,dla_name,bal,noise_estimate,suffix_out)
            pk_out = bookkeeping.return_pk_path(path_in,folder_name_out,snr_cut)
            corrections.correct_individual_pk_noise(pk_in,
                                                    pk_out,
                                                    qsocat,
                                                    noise_correction)
